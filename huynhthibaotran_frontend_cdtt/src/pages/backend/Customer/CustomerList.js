...
function CustomerList() {
  ...
  const [reLoad, setReLoad] = useState();
    ...
    useEffect(function(){
      (async function(){
        setLoad(true)
        const result = await CustomerServices.getAll(page);
    ...
    async function handleStatus(id) {
      const result = await CustomerServices.changeStatus(id)
      // alert(result.message)
      setReLoad(Date.now)
    }
      //------------pagination-------------
      ...
      <div className="page-header">
        ...
        <nav aria-label="breadcrumb">
          <ol className="breadcrumb">
            <li className="breadcrumb-item"><Link to='/admin/customer'>Tất cả({total})</Link></li>
            <li className="breadcrumb-item active" aria-current="page" >Xuất bản({publish})</li>
            <li className="breadcrumb-item active" aria-current="page"><Link to='/admin/customer/trash'>Rác({trash})</Link></li>
          </ol>
        </nav>
      </div>
      <div className="page-header">
                          ...
                          <td>
                            <div className='row'>
                              ...
                              <div className="col- 2 function_style">
                                <button onClick={() => handleStatus(user.id)} className={user.status === 1 ? "btn btn-success btn-sm" : "btn btn-danger btn-sm"}>
                                      <i className={user.status === 1 ? "fa fa-toggle-on" : "fa fa-toggle-off"} />
                                </button>
