...
function ProductTrashList() {
  ...
  useEffect(function () {
    (async function () {
      ...
      const resultProduct = await ProductServices.trash(page);
  ...
  async function handleReStore(id) {
    const result = await ProductServices.restore(id)
    // alert(result.message)
    setReLoad(Date.now)
  }
  //------------pagination-------------
      ...
      <tr className="datarow">
        ...
        <td>
          <div className='row'>
            ...
            <button onClick={()=>handleReStore(product.id)} className="btn btn-success btn-sm">
                                  <TbRestore />
                                </button>
