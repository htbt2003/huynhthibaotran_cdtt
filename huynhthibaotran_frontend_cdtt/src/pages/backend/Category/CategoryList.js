...
function CategoryList() {
  ...
  const [total, setTotal] = useState();
    ...
    useEffect (function(){
          (async function(){
            setLoad(true)
            const result = await CategoryServices.getAll(page);
    ...
    async function handleStatus(id) {
      const result = await CategoryServices.changeStatus(id)
      // alert(result.message)
      setReLoad(Date.now)
    }
      //------------pagination-------------
                          ...
                          <td>
                            <div className='row'>
                              ...
                              <div className="col- 2 function_style">
                                <button onClick={() => handleStatus(category.id)} className={category.status === 1 ? "btn btn-success btn-sm" : "btn btn-danger btn-sm"}>
                                      <i className={category.status === 1 ? "fa fa-toggle-on" : "fa fa-toggle-off"} />
                                </button>
